...
async function scrapeTrainInfo() {
  ...
  if ((currentHour < 7) || (currentHour === 7 && currentMinute < 45) || (currentHour > 8) || (currentHour === 8 && currentMinute > 15)) {
    return [];
  }
  ...
  try {
    ...
    if(info.filter(v => (v.scheduledTime.includes("07:59") || v.scheduledTime.includes("08:14"))).length > 0){
        return info.filter(v => v.departureStopsDisplay.includes("Union Station"))
    }
    return []
  } catch (error) {
    if (browser) await browser.close();
